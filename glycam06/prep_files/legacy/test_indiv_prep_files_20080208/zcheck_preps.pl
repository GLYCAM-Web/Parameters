...
my $trk_ano; #check anomeric configuration. esp for furanoses
my $trk_sug; #track sugar name. esp for furanoses
my $inputfile = "header.in";
my $run_tleap;
my (@RES, $i, $j);#$i & $j track residue names. $j just for access to 0?? residues
my @PREP;
my ($head, $tail);
		...
		if($_ =~ /INT/)
		{
			...
			#check number of branches
				...
				print INF "charge x";
                        ...
                        if((($trk_sub =~ /2/)and($trk_sug !~ /[CPBJ]/i))or (($trk_sub =~ /2/)and($trk_sug !~ /SA/)))
                        {
                                my $inf = "inputfile";
                                open(INF,">$inf");
                        ...
                        if((($trk_sug !~ /[CPBJ]/i)and($trk_sub =~ /1/))
                          or(($trk_sub =~ /2/)and($RES[$i] =~ /SA/))
                          or(($trk_sub =~ /2/)and($trk_sug =~ /[CPBJ]/i)))
                        {
                              my $inf = "inputfile";
                              open(INF,">$inf");
